...
export default function HomeCarousel({ data, istatColors = {}, istatLabels = {} }: HomeCarouselProps) {
  ...
  const gap = visibleCount === 1 ? 0 : visibleCount === 2 ? 24 : 32; // px
  const maxIndex = Math.max(0, total - visibleCount);

  // Reset position when visibleCount changes
  useEffect(() => {
        ...
        <div className="relative">
          <div className="overflow-hidden">
            <div
              ...
              style={{ transform: `translateX(calc(-${current} * (100% / ${visibleCount}) - ${current} * (2rem / ${visibleCount})))` }}
            >
              {data.map((item) => {
                const href = ROUTES.ergaSingle(item.id);
                return (
                  <article
                    key={item.id}
                    className="flex flex-col items-start justify-between shrink-0"
                    style={{ width: `calc((100% - ${(visibleCount - 1) * 2}rem) / ${visibleCount})` }}
                  >
                    {/* Image */}
                    <div className="relative w-full">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
